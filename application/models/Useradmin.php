...
class Useradmin extends CI_Model{
	...
	function login_admin($username,$password){
		...
		return $this->db->get('akademikrpl');
	}
	//
	...
	//Join Tabel Nilai
	function get_data_cond_join_nilai($where,$like,$orderby,$ordertype,$offset,$jml){

		$this->db->select('nilai.*,siswa.*');
		$this->db->from('nilai');
		$this->db->join('siswa','siswa.id = nilai.id_siswa','inner');
	...
	function cek_siswa($where){		
		$this->db->where($where);
		return $this->db->get('siswa');
	}

	function cek_nilai($where){		
		$this->db->where($where);
		return $this->db->get('nilai');
	}

	function get_gen_siswa($where){
		$this->db->where($where);
		return $this->db->get('siswa');
	}
}